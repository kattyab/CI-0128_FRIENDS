...
public class ApiDeductionTest
{
    ...
    public void Setup()
    {
        ...
        // Use a real context with a fake connection string (no connection is opened)
        _context = new PayrollTransactionContext("Server=(local);Database=FakeDb;Trusted_Connection=True;");
    ...
    public async Task GetDeductionsForEmployeeAsyncReturnsCorrectDeductions()
    {
        ...
        _mockRepository.Setup(r => r.GetBenefitsAsync(_companyId, _context)).ReturnsAsync(benefits);
        _mockRepository.Setup(r => r.GetParametersForCompanyAsync(_companyId, _context)).ReturnsAsync(parameters);

        _mockApiCaller.Setup(api => api.FetchDeductionAsync(benefits[0],
            It.Is<Dictionary<string, string>>(d => d["param1"] == "value1")))
            .ReturnsAsync(100m);

        _mockApiCaller.Setup(api => api.FetchDeductionAsync(benefits[1],
            It.Is<Dictionary<string, string>>(d => d["param2"] == "value2")))
            .ReturnsAsync(200m);

        var result = await _service.GetDeductionsForEmployeeAsync(employeeId, _context);
    ...
    public async Task GetDeductionsForEmployeeAsyncMissingParametersOmitsBenefit()
    {
        ...
        _mockRepository.Setup(r => r.GetBenefitsAsync(_companyId, _context)).ReturnsAsync(benefits);
        _mockRepository.Setup(r => r.GetParametersForCompanyAsync(_companyId, _context))
    ...
    public async Task GetDeductionsForEmployeeAsyncFiltersParametersByEmployee()
    {
        ...
        _mockApiCaller.Setup(api => api.FetchDeductionAsync(benefit,
            It.Is<Dictionary<string, string>>(d => d["param1"] == "correctvalue")))
            .ReturnsAsync(100m);
