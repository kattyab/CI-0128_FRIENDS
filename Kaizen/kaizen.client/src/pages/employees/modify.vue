  ...
  <div class="page container-fluid">
    ...
    <div class="row mb-4">
      <div class="mb-3 col-10">
        <label for="searchEmail" class="form-label">Buscar Empleado por Email</label>
    ...
    <!-- Employee form (only shown when data is loaded) -->
    <form v-if="employeeLoaded" class="has-validation" @submit.prevent="updateEmployee">
      <div class="row ">
        <div class="mb-3 col-10">
      ...
      <div class="row">
        <div class="mb-3 col-4">
          <button type="submit" class="btn btn-primary btn-lg btn-block" :disabled="isUpdating">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      bodyTextHTML() {
        return `<span>Empleado actualizado exitosamente</span>`;
      },
      ...
      isPasswordValid() {
        // Password is optional for editing, so if empty it's valid
    ...
    methods: {
      async fetchEmployeeData() {
        if (!this.searchEmail || !this.searchEmail.trim()) {
          this.searchError = 'Por favor ingrese un email válido';
          return;
        }
        ...
        try {
          const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/employee/getByEmail`, {
            params: { email: this.searchEmail.trim() },
          ...
          this.originalEmployeeId = employeeData.id;
          this.name = employeeData.name || '';
          this.lastname = employeeData.lastname || '';
          this.personid = employeeData.personid || '';
          this.idType = employeeData.idType || 'Cédula';
          this.birthdate = employeeData.birthdate ? employeeData.birthdate.split('T')[0] : '';
          this.sex = employeeData.sex || '';
          this.province = employeeData.province || '';
          this.canton = employeeData.canton || '';
          this.othersigns = employeeData.othersigns || '';
          this.phonenumber = employeeData.phonenumber || '';
          this.role = employeeData.role || '';
          this.jobposition = employeeData.jobposition || '';
          this.contract = employeeData.contract || '';
          this.paycycle = employeeData.paycycle || '';
          this.brutesalary = employeeData.brutesalary ? employeeData.brutesalary.toString() : '';
          this.startdate = employeeData.startdate ? employeeData.startdate.split('T')[0] : '';
          this.bankaccount = employeeData.bankaccount || '';
          ...
          this.searchError = error.response?.data?.message || 'No se encontró el empleado o error en el servidor';
        } finally {
      ...
      async updateEmployee() {
        ...
        try {
          ...
          const response = await axios.put(`${import.meta.env.VITE_API_URL}/api/employee/update`, updateData, {
            withCredentials: true
          });

          console.log("Successfully updated employee", response);
          this.titletext = "Empleado actualizado con éxito";
          this.showNotification = true;

        } catch (error) {
          this.formErrorMessage = error.response?.data?.message || 'Error actualizando al empleado. Inténtelo más tarde.';
