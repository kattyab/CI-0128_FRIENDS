  ...
  <div class="container" ref="exportContainer">
    <div class="header">
      ...
      <div class="employee-info" ref="headerInfo">
        <div class="info-item">
          ...
          <span class="info-value">{{ currentPayrollData?.companyName || 'Cargando...' }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Nombre del empleado:</span>
          <span class="info-value">{{ currentPayrollData?.employeeFullName || 'Cargando...' }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Tipo de contrato:</span>
          <span class="info-value">{{ currentPayrollData?.contractType || 'Cargando...' }}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Fecha de pago:</span>
          <span class="info-value">{{ formatPayDate(currentPayrollData?.payDate) }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="payroll-content" v-if="currentPayrollData" ref="payrollContent">
      <div class="payroll-section">
        <h3 class="section-title" @click="toggleSection('salary')" style="cursor: pointer;">
          Salario Bruto
          <span class="toggle-arrow">{{ collapsedSections.salary ? '▶' : '▼' }}</span>
        </h3>
        <table v-if="!collapsedSections.salary" class="payroll-table">
          <tbody>
            <tr class="payroll-row total-row">
              <td class="payroll-cell">Salario bruto</td>
              <td class="payroll-cell amount total-amount bold">
                <span class="currency">₡</span>{{ formatAmount(currentPayrollData.bruteSalary) }}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="payroll-section">
        <h3 class="section-title" @click="toggleSection('obligatoryDeductions')" style="cursor: pointer;">
          Deducciones Obligatorias
          <span class="toggle-arrow">{{ collapsedSections.obligatoryDeductions ? '▶' : '▼' }}</span>
        </h3>
        <table v-if="!collapsedSections.obligatoryDeductions" class="payroll-table">
          <tbody>
            <tr class="payroll-row total-row">
              <td class="payroll-cell">Total deducciones obligatorias</td>
              <td class="payroll-cell amount total-amount bold">
                <span class="currency">₡</span>{{ formatAmount(currentPayrollData.totalObligatoryDeductions) }}
              </td>
            </tr>
            <tr class="payroll-row"
                v-for="deduction in obligatoryDeductions"
                :key="deduction.key">
      ...
      <div class="payroll-section" v-if="currentPayrollData.optionalDeductions.length > 0">
        <h3 class="section-title" @click="toggleSection('optionalDeductions')" style="cursor: pointer;">
          Deducciones Voluntarias
          <span class="toggle-arrow">{{ collapsedSections.optionalDeductions ? '▶' : '▼' }}</span>
        </h3>
        <table v-if="!collapsedSections.optionalDeductions" class="payroll-table">
          <tbody>
            <tr class="payroll-row total-row">
              <td class="payroll-cell">Total deducciones voluntarias</td>
              <td class="payroll-cell amount total-amount bold">
                <span class="currency">₡</span>{{ formatAmount(currentPayrollData.totalOptionalDeductions) }}
              </td>
            </tr>
            <tr class="payroll-row"
                v-for="deduction in currentPayrollData.optionalDeductions"
                :key="deduction.name">
              <td class="payroll-cell">{{ deduction.name }}</td>
              <td class="payroll-cell amount">
                <span class="currency">₡</span>{{ formatAmount(deduction.amount) }}
              </td>
            </tr>
      ...
      <div class="payroll-section">
        <h3 class="section-title" @click="toggleSection('netPay')" style="cursor: pointer;">
          Salario Neto
          <span class="toggle-arrow">{{ collapsedSections.netPay ? '▶' : '▼' }}</span>
        </h3>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <p class="modal-description">
            Seleccione cómo desea exportar el reporte de empleado para el período {{ formatPeriodDisplay(selectedPeriod) }}:
          </p>
...
<script setup>
  ...
  const API_ENDPOINT_TEMPLATE = '/api/Reports/employee/'

  const ERROR_MESSAGES = {
    GUID_NOT_FOUND: 'Employee GUID not found in URL',
    FETCH_DEFAULT: 'Error fetching employee payroll data'
  }

  const OBLIGATORY_DEDUCTION_CONFIGS = [
    { key: 'sem', label: 'SEM (Seguro de Enfermedad y Maternidad)' },
    { key: 'ivm', label: 'IVM (Invalidez, Vejez y Muerte)' },
    {
      key: 'employeeAportacionBancoPopular',
      label: 'Aportación Banco Popular'
    },
    { key: 'incomeTax', label: 'Impuesto sobre la renta' }
  ]

  const SECTION_NAMES = {
    SALARY: 'salary',
    OBLIGATORY_DEDUCTIONS: 'obligatoryDeductions',
    OPTIONAL_DEDUCTIONS: 'optionalDeductions',
    NET_PAY: 'netPay'
  }
  ...
  const employeeGuid = ref('')

  const collapsedSections = ref({
    [SECTION_NAMES.SALARY]: false,
    [SECTION_NAMES.OBLIGATORY_DEDUCTIONS]: false,
    [SECTION_NAMES.OPTIONAL_DEDUCTIONS]: false,
    [SECTION_NAMES.NET_PAY]: false
  })
  ...
  const obligatoryDeductions = computed(() => {
    if (!currentPayrollData.value) return []

    const data = currentPayrollData.value
    return OBLIGATORY_DEDUCTION_CONFIGS.map(config => ({
      key: config.key,
      label: config.label,
  ...
  async function fetchEmployeePayrollData() {
    try {
      loading.value = true
      error.value = null
  ...
  async function handleEmail() {
    ...
    periodTitle.textContent = `Reporte de Empleado - ${formatPeriodDisplay(selectedPeriod.value)}`
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const emailPayload = {
          to: [userEmail.value],
          subject: `Reporte de Empleado - ${formatPeriodDisplay(selectedPeriod.value)}`,
          body: 'Adjunto encontrará el reporte de empleado en formato PDF.',
          fileName: `reporte_empleado_${selectedPeriod.value}.pdf`,
