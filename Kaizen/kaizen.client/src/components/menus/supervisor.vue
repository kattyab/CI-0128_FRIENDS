<template>
  <aside :class="{ is_expanded }" class="d-flex flex-column p-3">
...
<script setup>
  ...
  const fetchCompanyId = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/ReportMenu/employee-id`)
      employee_pk.value = response.data
    } catch (error) {
